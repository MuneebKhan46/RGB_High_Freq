...
class MobileNetV3(models.Model):
    def __init__(self, input_shape=(224, 224, 3), model_type='large'):
...
mobNet_wcw_model = MobileNetV3(input_shape=(224, 224, 3), model_type='large')
mobNet_wcw_model.build(input_shape=(None, 224, 224, 3))

mobNet_wcw_model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
    
wcw_model_checkpoint = keras.callbacks.ModelCheckpoint(filepath='/WACV_Paper/Models/MobileNet_RGB_wCW.keras', save_best_only=True, monitor='val_accuracy', mode='max', verbose=1)
...
wcw_history_df.to_csv('/WACV_Paper/History_RGB/MobileNet_RGB_wCW.csv', index=False)
...
mobNet_cw_model = MobileNetV3(input_shape=(224, 224, 3), model_type='large')
mobNet_cw_model.build(input_shape=(None, 224, 224, 3))

mobNet_cw_model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])

cw_model_checkpoint = keras.callbacks.ModelCheckpoint(filepath='/WACV_Paper/Models/MobileNet_RGB_CW.keras', save_best_only=True, monitor='val_accuracy', mode='max', verbose=1)
...
cw_history_df.to_csv('/WACV_Paper/History_RGB/MobileNet_RGB_CW.csv', index=False)
...
file_path = '/WACV_Paper/Plots_CSV/MobileNet_RGB_wCW_PR_Curve.csv'
...
plt.legend()
plt.grid(True)
precision_recall_curve_path = '/WACV_Paper/Plots_RGB/MobileNet_RGB_wCW_PR_Curve.png'
...
pr_data = pd.DataFrame({'Precision': precision, 'Recall': recall })
file_path = '/WACV_Paper/Plots_CSV/MobileNet_RGB_CW_PR_Curve.csv'
pr_data.to_csv(file_path, index=False)
...
plt.legend()
plt.grid(True)
precision_recall_curve_path = '/WACV_Paper/Plots_RGB/MobileNet_Diff_CW_PR_Curve.png'
